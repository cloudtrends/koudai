					...
					<?php foreach ($permissions as $controller => $permission): ?>
						<tr>
							<th class="partition" colspan="15">
								<label><input type="checkbox" onclick="permcheckall(this, '<?php echo $controller; ?>')" class="checkbox"> <?php echo $permission['label']; ?></label>
							</th>
						</tr>
						<tr>
							<td id="<?php echo $controller; ?>" class="vtop">
								...
								<div class="item<?php echo in_array($action->route, $permissionChecks) ? ' checked' : ''; ?>">
									<label class="txt"><input type="checkbox" onclick="checkclk(this)" class="checkbox" value="<?php echo $action->route; ?>" name="permissions[]"<?php echo in_array($action->route, $permissionChecks) ? ' checked' : ''; ?>><?php echo $action->title; ?></label>
								</div>
								<?php endforeach; ?>
							</td>
						</tr>
